...
export const useContactsStore = defineStore("contacts", () => {
  ...
  const hasMoreContacts = computed(
    () => contacts.value.length < contactsTotal.value,
  );
    ...
    try {
      ...
      if (newContacts.length > 0) {
        ...
        const uniqueNew = newContacts.filter(
          (c: Contact) => !existingIds.has(c.id),
        );
  ...
  async function fetchOlderMessages(contactId: string, account?: string) {
    ...
    try {
      ...
      const response = await messagesService.list(contactId, {
  ...
  async function sendMessage(
    ...
    extra?: {
      interactive?: Parameters<typeof messagesService.send>[1]["interactive"];
    },
  ) {
    try {
  ...
  async function sendTemplate(
    ...
    try {
      const response = await messagesService.sendTemplate(
        contactId,
        {
