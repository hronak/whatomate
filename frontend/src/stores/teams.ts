...
export const useTeamsStore = defineStore("teams", () => {
  ...
  async function createTeam(data: CreateTeamData): Promise<Team> {
    ...
    try {
      ...
      const newTeam = response.data?.team || response.data?.team;
  ...
  async function updateTeam(id: string, data: UpdateTeamData): Promise<Team> {
    ...
    try {
      ...
      const updatedTeam =
        response.data?.team || response.data?.team;
  ...
  async function fetchTeamMembers(teamId: string): Promise<TeamMember[]> {
    try {
      ...
      return (
        response.data?.members || response.data?.members || []
      );
    } catch (err: any) {
      error.value =
  ...
  async function addTeamMember(
    teamId: string,
    userId: string,
    role: "manager" | "agent" = "agent",
  ): Promise<TeamMember> {
    try {
      const response = await teamsService.addMember(teamId, {
        user_id: userId,
        role,
      });
      // Update member count
      ...
      return response.data?.member || response.data?.member;
    } catch (err: any) {
