...
export const useCallingStore = defineStore("calling", () => {
  ...
  const callPermissions = reactive(
    new Map<string, { status: string; expiresAt?: string }>(),
  );

  // Outgoing call state
  const outgoingCallLogId = ref<string | null>(null);
  const outgoingCallStatus = ref<
    "initiating" | "ringing" | "answered" | "ended" | null
  >(null);
  ...
  const activeFlows = computed(() =>
    ivrFlows.value.filter((f) => f.is_active && f.is_call_start),
  );
  ...
  // IVR Flows actions
  async function fetchIVRFlows(params?: {
    search?: string;
    page?: number;
    limit?: number;
  }) {
  ...
  async function createIVRFlow(
    flowData: Parameters<typeof ivrFlowsService.create>[0],
  ) {
  ...
  async function updateIVRFlow(
    id: string,
    flowData: Parameters<typeof ivrFlowsService.update>[1],
  ) {
  ...
  async function fetchWaitingTransfers() {
    try {
      ...
      waitingTransfers.value =
        data.data?.call_transfers ?? data.call_transfers ?? [];
    } catch {
      // Silently handle
    }
  }
  ...
  async function initiateTransfer(teamId: string, agentId?: string) {
    const callLogId =
      outgoingCallLogId.value ?? activeTransfer.value?.call_log_id;
  ...
  function handleCallEvent(type: string, payload: any) {
    switch (type) {
      ...
      case "call_transfer_connected":
        ...
        waitingTransfers.value = waitingTransfers.value.filter(
          (t) => t.id !== payload.id,
        );
        break;
