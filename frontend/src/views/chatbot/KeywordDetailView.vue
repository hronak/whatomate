...
import { Key, Trash2, Save, Plus } from "@lucide/vue";
...
const canWrite = computed(() =>
  authStore.hasPermission("chatbot.keywords", "write"),
);
const canDelete = computed(() =>
  authStore.hasPermission("chatbot.keywords", "delete"),
);
...
const breadcrumbs = computed(() => [
  ...
  {
    label: isNew.value
      ? t("keywords.newKeyword", "New Keyword")
      : keyword.value?.name || form.value.keywords.split(",")[0]?.trim() || "",
  },
]);
...
async function save() {
  if (!form.value.keywords.trim()) {
    toast.error(
      t("keywords.enterKeyword", "Please enter at least one keyword"),
    );
    return;
  }

  if (
    form.value.response_type !== "transfer" &&
    !form.value.response_content.trim()
  ) {
    ...
    if (isNew.value) {
      ...
      toast.success(
        t("common.createdSuccess", {
          resource: t("resources.KeywordRule", "Keyword Rule"),
        }),
      );
...
<template>
  <div class="h-full">
    <DetailPageLayout
      :title="
        isNew
          ? $t('keywords.newKeyword', 'New Keyword Rule')
        ...
        <CardHeader class="pb-3">
          <div class="flex items-center justify-between">
            <CardTitle class="font-medium">{{
              $t("keywords.details", "Details")
            }}</CardTitle>
            <Badge
              :variant="
                (keyword?.enabled ?? form.enabled) ? 'default' : 'secondary'
              "
            >
              {{
                (keyword?.enabled ?? form.enabled)
                  ? $t("keywords.active", "Active")
                  : $t("keywords.inactive", "Inactive")
              }}
            </Badge>
          </div>
        </CardHeader>
        <CardContent class="space-y-4">
          <div class="space-y-1.5">
            ...
            <Input
              ...
              :placeholder="
                $t('keywords.keywordsPlaceholder', 'hello, hi, hey')
              "
              :disabled="!canWrite"
            />
            <p class="text-muted-foreground">
              {{
                $t("keywords.keywordsHint", "Comma-separated list of keywords")
              }}
            </p>
          </div>
          <div class="space-y-1.5">
            ...
            <Select v-model="form.match_type" :disabled="!canWrite">
              ...
              <SelectContent>
                <SelectItem value="contains">{{
                  $t("keywords.contains", "Contains")
                }}</SelectItem>
                <SelectItem value="exact">{{
                  $t("keywords.exact", "Exact")
                }}</SelectItem>
                <SelectItem value="regex">{{
                  $t("keywords.regex", "Regex")
                }}</SelectItem>
              </SelectContent>
            </Select>
          </div>
            ...
            <Select v-model="form.response_type" :disabled="!canWrite">
              ...
              <SelectContent>
                <SelectItem value="text">{{
                  $t("keywords.textResponse", "Text Response")
                }}</SelectItem>
                <SelectItem value="transfer">{{
                  $t("keywords.transferToAgent", "Transfer to Agent")
                }}</SelectItem>
              </SelectContent>
            </Select>
          </div>
          <div class="space-y-1.5">
            <Label>
              {{
                form.response_type === "transfer"
                  ? $t("keywords.transferMessage", "Transfer Message")
                  : $t("keywords.responseMessage", "Response Message")
              }}
            ...
            <Textarea
              ...
              :placeholder="
                form.response_type === 'transfer'
                  ? $t(
                      'keywords.transferPlaceholder',
                      'Optional message before transfer',
                    ) + '...'
                  : $t(
                      'keywords.responsePlaceholder',
            ...
            />
            <p
              v-if="form.response_type === 'transfer'"
              class="text-muted-foreground"
            >
              {{
                $t(
                  "keywords.transferHint",
                  "Optional message sent before transferring to a human agent",
                )
              }}
            </p>
          </div>
          ...
          <div v-if="form.response_type === 'text'" class="space-y-1.5">
            ...
            <p class="text-muted-foreground">
              {{
                $t(
                  "keywords.buttonsHint",
                  "Add quick-reply buttons to the response message",
                )
              }}
            </p>
    ...
    <AlertDialog v-model:open="deleteDialogOpen">
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>{{
            $t("keywords.deleteRule", "Delete Keyword Rule")
          }}</AlertDialogTitle>
          <AlertDialogDescription>
            {{
              $t(
                "keywords.deleteRuleDesc",
        ...
        <AlertDialogFooter>
          ...
          <AlertDialogAction @click="deleteKeyword">{{
            $t("common.delete")
          }}</AlertDialogAction>
        </AlertDialogFooter>
      </AlertDialogContent>
