...
const assignedUserName = computed(() => {
  ...
  const user = usersStore.users.find(
    (u) => u.id === form.value.assigned_user_id,
  );
      ...
      <Card>
        <CardHeader class="pb-3">
          <div class="flex items-center justify-between">
            ...
            <div class="flex items-center gap-2">
              <Badge v-if="contact?.marketing_opt_out" variant="secondary">{{
                $t("contacts.marketingOptOut", "Marketing Opt-out")
              }}</Badge>
            </div>
          </div>
        </CardHeader>
        <CardContent class="space-y-4">
            ...
            <Popover v-model:open="agentSelectorOpen">
              <PopoverTrigger as-child>
                ...
                >
                  <span
                    ...
                    class="text-muted-foreground"
                    >{{ $t("contacts.selectAgent", "Select agent") }}</span
                  >
              ...
              >
                <Command>
                  <CommandInput
                    :placeholder="
                      $t('contacts.searchAgents', 'Search agents...')
                    "
                  />
                  <CommandList>
                    <CommandEmpty>{{
                      $t("contacts.noAgentsFound", "No agents found")
                    }}</CommandEmpty>
                    <CommandGroup>
                      <CommandItem
                      ...
                      >
                        <X class="size-3.5" />
                        <span>{{
                          $t("contacts.removeAssignment", "Remove assignment")
                        }}</span>
                      </CommandItem>
                      <CommandItem
                      ...
                      >
                        ...
                        <Check
                          v-if="form.assigned_user_id === u.id"
                          class="size-4 text-primary"
                        />
                      </CommandItem>
                    </CommandGroup>
          ...
          <div class="flex items-center justify-between">
            ...
            <Badge
              :variant="
                contact?.marketing_opt_out ? 'destructive' : 'secondary'
              "
            >
              {{
