...
const canWrite = computed(() =>
  authStore.hasPermission("canned_responses", "write"),
);
const canDelete = computed(() =>
  authStore.hasPermission("canned_responses", "delete"),
);
...
const buttonsValidationError = computed<string | null>(() => {
  ...
  if (flow.length > 1) {
    return t(
      "cannedResponses.errorMultiFlow",
      "Only one Flow button is allowed per message.",
    );
  }
  ...
  if (flow.length === 1) {
    ...
    if (!f.title?.trim()) {
      return t(
        "cannedResponses.errorFlowTitle",
        "The Flow button needs a label (shown on the button face).",
      );
    }
    if (!f.flow_id) {
      return t(
        "cannedResponses.errorFlowId",
        "Select a published flow for the Flow button.",
      );
    }
  }
  if (phone.length > 0) {
    ...
    <AlertDialog v-model:open="deleteDialogOpen">
      <AlertDialogContent>
        ...
        <AlertDialogFooter>
          ...
          <AlertDialogAction @click="deleteResponse">{{
            $t("common.delete")
          }}</AlertDialogAction>
        </AlertDialogFooter>
      </AlertDialogContent>
