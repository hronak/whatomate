...
const {
  ...
  openDeleteDialog,
  closeDeleteDialog,
} = useCrudState<Team, Record<string, never>>({});
...
const canDeleteTeams = computed(() =>
  authStore.hasPermission("teams", "delete"),
);
...
<template>
  <div class="flex flex-col h-full bg-background">
      ...
      <template #actions>
        <RouterLink v-if="canWriteTeams" to="/settings/teams/new"
          ><Button variant="outline" size="sm"
            ><Plus class="size-4 mr-2" />{{ $t("teams.addTeam") }}</Button
          ></RouterLink
        >
      </template>
    </PageHeader>
    ...
    >
      <template #action
        ><Button size="sm" @click="fetchTeams">{{
          $t("common.retry")
        }}</Button></template
      >
    </ErrorState>
                      ...
                      ><TooltipTrigger as-child
                        ><RouterLink :to="`/settings/teams/${team.id}`"
                          ><Button variant="ghost" size="icon" class="size-8"
                            ><Pencil
                              class="size-4" /></Button></RouterLink></TooltipTrigger
                      ><TooltipContent>{{
                    ...
                    <Tooltip v-if="canDeleteTeams"
                      ><TooltipTrigger as-child
                        ><Button
                          ...
                          ><Trash2
                            class="size-4 text-destructive" /></Button></TooltipTrigger
                      ><TooltipContent>{{
