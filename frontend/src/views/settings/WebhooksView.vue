...
const {
  ...
  itemToDelete: webhookToDelete,
  openDeleteDialog,
  closeDeleteDialog,
} = useCrudState<Webhook, Record<string, never>>({});
      ...
      <template #actions>
        <RouterLink v-if="canWrite" to="/settings/webhooks/new">
          <Button variant="outline" size="sm"
            ><Plus class="size-4 mr-2" />{{ $t("webhooks.addWebhook") }}</Button
          >
        </RouterLink>
      </template>
    </PageHeader>
              ...
              >
                ...
                <template #cell-events="{ item: webhook }">
                  <div class="flex flex-wrap gap-1">
                    ...
                    >
                    <Badge v-if="webhook.events.length > 2" variant="outline"
                ...
                <template #cell-status="{ item: webhook }">
                  <div class="flex items-center gap-2">
                    <Switch
                      ...
                      @update:checked="handleToggleWebhook(webhook)"
                    />
                ...
                <template #cell-actions="{ item: webhook }">
                  <div class="flex items-center justify-end gap-1">
                    <IconButton
                      :icon="Play"
                      :label="$t('webhooks.testWebhook')"
                      class="size-8"
                      :disabled="isTesting === webhook.id"
                      :loading="isTesting === webhook.id"
