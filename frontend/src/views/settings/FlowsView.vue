...
const {
  ...
  openDeleteDialog,
  closeDeleteDialog,
} = useCrudState<WhatsAppFlow, Record<string, never>>({});
...
async function confirmSaveToMeta() {
  ...
  try {
    await flowsService.saveToMeta(flowToSaveToMeta.value.id);
    toast.success(t("flows.flowSavedToMeta"));
    saveToMetaDialogOpen.value = false;
    flowToSaveToMeta.value = null;
    await fetchFlows();
  } catch (e) {
...
async function confirmPublishFlow() {
  ...
  try {
    await flowsService.publish(flowToPublish.value.id);
...
<template>
  <div class="flex flex-col h-full bg-background">
    ...
    >
      <template #actions>
        <Button
          ...
          ><RefreshCw :class="['size-4 mr-2', isSyncing && 'animate-spin']" />{{
            $t("flows.syncFromMeta")
          }}</Button
        >
        <Button variant="outline" size="sm" @click="openCreateDialog"
          ><Plus class="size-4 mr-2" />{{ $t("flows.createFlow") }}</Button
        >
      </template>
    </PageHeader>
              ...
              >
                ...
                <template #cell-status="{ item: flow }">
                  ...
                  <Badge
                    v-else
                    variant="outline"
                    :class="[getStatusClass(flow.status), '']"
                    >{{ flow.status }}</Badge
                  >
                </template>
                ...
                <template #cell-actions="{ item: flow }">
                  <div class="flex items-center justify-end gap-1">
                    ...
                    <IconButton
                      ...
                      :icon="ExternalLink"
                      :label="$t('flows.previewTooltip')"
                      class="size-8"
                      @click="openPreviewUrl(flow.preview_url!)"
                    />
                    <IconButton
                      v-if="
    ...
    <Dialog v-model:open="showCreateDialog">
      <DialogContent class="max-w-6xl h-[85vh] flex flex-col">
        ...
        <div class="flex gap-4 py-2 border-b">
          ...
          <div class="flex items-center gap-2">
            <Label class="whitespace-nowrap">{{ $t("flows.name") }}:</Label
            ><Input
      ...
      <DialogContent class="max-w-6xl h-[85vh] flex flex-col">
        ...
        <DialogFooter
          ...
          ><Button size="sm" @click="updateFlow" :disabled="isUpdating"
            ><Spinner v-if="isUpdating" class="size-4 mr-2" />{{
              $t("flows.saveChanges")
            }}</Button
          ></DialogFooter
        >
    ...
    <ConfirmDialog
      ...
      :description="
        $t('flows.publishConfirmDescription', { name: flowToPublish?.name })
      "
    ...
    <ConfirmDialog
      ...
      :description="
        $t('flows.saveToMetaConfirmDescription', {
          name: flowToSaveToMeta?.name,
        })
      "
