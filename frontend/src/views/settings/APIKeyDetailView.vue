  ...
  try {
    ...
    toast.success(
      t("common.createdSuccess", {
        resource: t("resources.APIKey", "API Key"),
      }),
    );
  } catch (e) {
    toast.error(
...
async function toggleActive() {
  if (!apiKey.value) return;
  try {
    await apiKeysService.update(apiKey.value.id, {
      is_active: !apiKey.value.is_active,
    });
    toast.success(
      apiKey.value.is_active
  ...
  <div class="h-full">
        ...
        <CardContent class="space-y-4">
          <div class="flex items-center gap-3 p-3 rounded-lg bg-muted/50">
            ...
            <div class="min-w-0">
              ...
              <code
                class="text-muted-foreground font-mono bg-muted px-1.5 py-0.5 rounded"
                >whm_{{ apiKey?.key_prefix }}...</code
              >
            </div>
          </div>
          ...
          <div class="grid gap-4">
            <div class="space-y-1">
              ...
              <p>
                {{
                  apiKey?.created_at ? formatDateTime(apiKey.created_at) : "—"
                }}
              </p>
            </div>
            <div class="space-y-1">
      ...
      <DialogContent>
        ...
        <DialogFooter>
          <Button
            size="sm"
            @click="
              isKeyDisplayOpen = false;
              if (createdKeyId)
                router.replace(`/settings/api-keys/${createdKeyId}`);
            "
    ...
    <AlertDialog v-model:open="deleteDialogOpen">
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>{{
            $t("apiKeys.deleteKey", "Delete API Key")
          }}</AlertDialogTitle>
          <AlertDialogDescription>
            {{
              $t(
        ...
        <AlertDialogFooter>
          ...
          <AlertDialogAction @click="deleteApiKey">{{
            $t("common.delete")
          }}</AlertDialogAction>
        </AlertDialogFooter>
      </AlertDialogContent>
