...
async function fetchProfile() {
  ...
  try {
    ...
    toast.error(
      t("common.failedLoad", { resource: t("resources.businessProfile") }),
    );
  } finally {
...
async function saveProfile() {
  ...
  try {
    ...
    toast.success(
      t("common.updatedSuccess", { resource: t("resources.BusinessProfile") }),
    );
    ...
    const message =
      ...
      t("common.failedUpdate", { resource: t("resources.businessProfile") });
    toast.error(message);
  } finally {
...
async function handleFileChange(event: Event) {
  ...
  try {
    await api.post(
      `/accounts/${props.accountId}/business_profile/photo`,
      formData,
      {
        headers: {
    ...
    toast.success(
      t("common.updatedSuccess", { resource: t("resources.ProfilePhoto") }),
    );
    // Refresh
    await fetchProfile();
  } catch (error: any) {
    ...
    toast.error(
      error.response?.data?.message ||
        t("common.failedUpload", { resource: t("resources.profilePhoto") }),
    );
  } finally {
    ...
    <div v-else class="space-y-6">
      ...
      <div class="grid gap-6 md:grid-cols-2">
        ...
        <div class="md:col-span-2 flex items-center gap-4">
          <div
            class="group relative size-24 rounded-full bg-secondary flex items-center justify-center overflow-hidden border border-border cursor-pointer hover:ring-2 hover:ring-ring hover:ring-offset-2 hover:ring-offset-background"
          ...
          >
            ...
            <div
              v-if="!isUploading"
              class="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 group-hover:opacity-100 z-10"
            >
        ...
        <div class="md:col-span-2 space-y-2">
          ...
          />
          <p class="text-muted-foreground text-right">
            {{ profile.description.length }}/512
          </p>
        </div>
          ...
          <Select v-model="profile.vertical">
            <SelectTrigger>
              <SelectValue :placeholder="$t('businessProfile.selectCategory')">
                <template v-if="profile.vertical">{{
