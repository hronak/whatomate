...
<template>
  <div class="h-full">
    ...
    >
      ...
      <Card>
        <CardHeader class="pb-3">
          <div class="flex items-center justify-between">
            ...
            <div class="flex items-center gap-2">
              <Badge v-if="isSelf" variant="outline">{{
                $t("users.you")
              }}</Badge>
              <Badge v-if="user?.is_super_admin" variant="default">{{
                $t("users.superAdmin")
              }}</Badge>
              <Badge v-if="isMember" variant="secondary">{{
                $t("users.member")
              }}</Badge>
              <Badge
                :variant="(user?.is_active ?? true) ? 'default' : 'secondary'"
              >
                {{
                  (user?.is_active ?? true)
                    ? $t("common.active")
                ...
                <SelectValue :placeholder="$t('users.selectRole')">
                  <template v-if="form.role_id">
                    <span class="capitalize">{{
                      rolesStore.roles.find((r) => r.id === form.role_id)?.name
                    }}</span>
                  </template>
                </SelectValue>
              </SelectTrigger>
      ...
      <!-- Activity Log -->
      <AuditLogPanel v-if="user" resource-type="user" :resource-id="user.id" />
