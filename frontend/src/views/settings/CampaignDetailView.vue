...
const isDraft = computed(
  () => isNew.value || campaign.value?.status === "draft",
);
...
const canStart = computed(() => {
  ...
  return (
    (s === "draft" || s === "scheduled" || s === "paused") &&
    (campaign.value?.total_recipients || 0) > 0
  );
});
...
const canCancel = computed(() => {
  const s = campaign.value?.status;
  return (
    s === "running" || s === "paused" || s === "processing" || s === "queued"
  );
});
...
// --- Header media state ---
const selectedTemplateHeaderType = computed(
  () => selectedTemplate.value?.header_type,
);
const {
  file: mediaFile,
    ...
    if (isNew.value) {
      ...
      if (mediaFile.value && created?.id) {
        try {
          ...
          toast.error(
            t("campaigns.mediaUploadFailed", "Failed to upload media"),
          );
        }
      }
      hasChanges.value = false;
...
async function confirmStartCampaign() {
  ...
  try {
    ...
    toast.error(
      getErrorMessage(
        err,
        t("campaigns.startFailed", "Failed to start campaign"),
      ),
    );
  } finally {
...
async function confirmPauseCampaign() {
  ...
  try {
    ...
    toast.error(
      getErrorMessage(
        err,
        t("campaigns.pauseFailed", "Failed to pause campaign"),
      ),
    );
  } finally {
...
async function confirmCancelCampaign() {
  ...
  try {
    ...
    toast.error(
      getErrorMessage(
        err,
        t("campaigns.cancelFailed", "Failed to cancel campaign"),
      ),
    );
  }
}
  ...
  try {
    ...
    toast.success(
      t(
        ...
        { count: result?.retry_count || 0 },
        `Retrying ${result?.retry_count || 0} failed messages`,
      ),
    );
    await loadCampaign();
    ...
    toast.error(
      getErrorMessage(err, t("campaigns.retryFailedError", "Failed to retry")),
    );
  }
}
...
async function loadExistingMedia() {
  ...
  try {
    ...
    const blob = new Blob([response.data], {
      type: (response.headers as any)["content-type"],
    });
...
const manualInputValidation = computed(() => {
  ...
  if (lines.length === 0) {
    return {
      ...
      totalLines: 0,
      validLines: 0,
      invalidLines: [] as { lineNumber: number; reason: string }[],
    };
  }
...
async function addRecipients() {
  ...
  if (lines.length === 0) {
    toast.error(
      t("campaigns.enterPhoneNumber", "Please enter at least one phone number"),
    );
    return;
  }
  ...
  try {
    ...
    toast.success(
      t(
        ...
        `Added ${result?.added_count || recipientsList.length} recipients`,
      ),
    );
    ...
    toast.error(
      getErrorMessage(
        err,
        t("campaigns.addRecipientsFailed", "Failed to add recipients"),
      ),
    );
  } finally {
...
function downloadSampleCSV() {
  ...
  const exampleRow = cols.map((c, idx) => {
    ...
    if (c === "header")
      return templateHeaderParamName.value
...
async function addRecipientsFromCSV() {
  ...
  try {
    ...
    if (lines.length < 2) {
      toast.error(
        t("campaigns.csvEmpty", "CSV file is empty or has no data rows"),
      );
      isAddingRecipients.value = false;
      return;
    }
      ...
      if (bodyNames.length > 0) {
        ...
        for (const paramName of bodyNames) {
          const colIdx = headers.findIndex(
            (h, idx) => !usedIndices.has(idx) && h === paramName.toLowerCase(),
          );
          if (colIdx !== -1) {
    ...
    if (recipientsList.length === 0) {
      toast.error(
        t("campaigns.noValidRowsToImport", "No valid rows found in CSV"),
      );
      isAddingRecipients.value = false;
      return;
    }
    ...
    toast.success(
      t(
        ...
        { count: result?.added_count || recipientsList.length },
        `Imported ${result?.added_count || recipientsList.length} recipients from CSV`,
      ),
    );
    showAddRecipientsDialog.value = false;
    csvFile.value = null;
  ...
  <div class="h-full">
    ...
    >
        ...
        <CardContent class="gap-y-4">
          <div class="gap-y-1.5">
            ...
            <Input
              ...
              :disabled="!isDraft"
              :placeholder="
                $t('campaigns.namePlaceholder', 'Enter campaign name')
              "
            />
          </div>
          <div class="gap-y-1.5">
          ...
          <div class="gap-y-1.5">
            ...
            >
              ...
              <SelectContent>
                <SelectItem
                  v-for="tmpl in templates.filter((t) => t.id)"
                  :key="tmpl.id"
                  :value="tmpl.id"
                >
          ...
          <div v-if="templateNeedsMedia" class="gap-y-1.5">
            ...
            <div
              v-if="campaign?.header_media_filename && !mediaFile"
              class="rounded-lg border overflow-hidden"
            >
              <!-- Image preview -->
              <img
                v-if="
                  campaign.header_media_mime_type?.startsWith('image/') &&
                  existingMediaUrl
                "
              ...
              <video
                v-else-if="
                  campaign.header_media_mime_type?.startsWith('video/') &&
                  existingMediaUrl
                "
                :src="existingMediaUrl"
                controls
              ...
              <div class="flex items-center gap-2 px-3 py-2 bg-muted/50">
                ...
                <span class="truncate flex-1">{{
                  campaign.header_media_filename
                }}</span>
                <Button
                  v-if="isDraft"
                  variant="ghost"
      ...
      <Card v-if="!isNew && campaign">
        <CardHeader class="pb-3">
          <CardTitle class="font-medium">{{
            $t("campaigns.statistics", "Statistics")
          }}</CardTitle>
        </CardHeader>
        <CardContent>
          <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
            <div class="flex flex-col items-center gap-1 rounded-lg border p-3">
              ...
              <span class="text-muted-foreground uppercase tracking-wide">{{
                $t("campaigns.totalRecipients", "Recipients")
              }}</span>
            </div>
            <div class="flex flex-col items-center gap-1 rounded-lg border p-3">
          ...
          <div v-if="campaign.total_recipients > 0" class="mt-4 gap-y-2">
            ...
            >
              ...
              <div
                ...
                :style="{
                  width: `${((campaign.sent_count - campaign.delivered_count) / campaign.total_recipients) * 100}%`,
                }"
              />
              <div
            ...
            <div class="flex items-center gap-4 text-muted-foreground">
              ...
              <span class="flex items-center gap-1"
                ...
                {{ $t("campaigns.sent", "Sent") }}</span
              >
              <span class="flex items-center gap-1"
                ><span class="size-2 rounded-full bg-destructive" />
                {{ $t("campaigns.failed", "Failed") }}</span
              >
            </div>
          </div>
        </CardContent>
      ...
      <Card v-if="!isNew && campaign">
        <Collapsible
          :default-open="recipients.length > 0 && recipients.length <= 20"
        >
          <CardHeader class="pb-3 flex flex-row items-center justify-between">
            ...
            >
              <ChevronDown
                class="size-4 text-muted-foreground in-data-[state=closed]:-rotate-90"
              />
              <CardTitle class="font-medium">
            ...
            <CardContent>
              ...
              <div v-else-if="recipients.length === 0" class="text-center py-8">
                ...
                <p class="text-muted-foreground">
                  {{ $t("campaigns.noRecipients", "No recipients yet") }}
                </p>
                <Button
                  v-if="isDraft"
                  variant="outline"
              ...
              <div v-else class="overflow-auto max-h-96">
                <Table>
                  <TableHeader>
                    <TableRow>
                      <TableHead>{{
                        $t("campaigns.phoneNumber", "Phone Number")
                      }}</TableHead>
                      <TableHead>{{
                        $t("campaigns.recipientName", "Name")
                      }}</TableHead>
                      <TableHead>{{
                        $t("campaigns.status", "Status")
                      }}</TableHead>
                      <TableHead>{{
                        $t("campaigns.sentAt", "Sent At")
                      }}</TableHead>
                      ...
                      <TableCell>
                        <div class="flex flex-col gap-0.5">
                          <Badge
                            variant="outline"
                            :class="[
                              getRecipientStatusClass(recipient.status),
                              'w-fit',
                            ]"
                          >
                            {{ recipient.status }}
    ...
    <AlertDialog v-model:open="deleteDialogOpen">
      <AlertDialogContent>
        ...
        <AlertDialogFooter>
          ...
          <AlertDialogAction @click="deleteCampaign">{{
            $t("common.delete")
          }}</AlertDialogAction>
        </AlertDialogFooter>
      </AlertDialogContent>
    ...
    <ConfirmDialog
      ...
      :description="
        $t(
          'campaigns.startConfirm',
          'This will begin sending messages to all recipients. Continue?',
        )
      "
    ...
    <ConfirmDialog
      ...
      :description="
        $t(
          'campaigns.pauseConfirm',
          'This will pause sending messages. You can resume later.',
        )
      "
    ...
    <ConfirmDialog
      ...
      :description="
        $t(
          'campaigns.cancelConfirm',
          'This will permanently stop the campaign. Messages already sent cannot be recalled.',
        )
      "
      ...
      <DialogContent class="sm:max-w-[550px]">
        <DialogHeader>
          ...
          <DialogDescription>
            {{
              $t(
                "campaigns.addRecipientsDescription",
          ...
          <TabsContent value="manual" class="gap-y-3 mt-3">
            <div class="gap-y-1.5">
              ...
              <p class="text-muted-foreground mb-1">
                {{
                  $t(
                    "campaigns.recipientFormatNote",
                    "One recipient per line. Name is optional. Template parameters must match the selected template.",
                  )
                }}
              </p>
              <Textarea
            ...
            <div v-if="recipientsInput.trim()">
              ...
              >
                ...
                <ul class="list-disc list-inside text-destructive gap-y-0.5">
                  ...
                  >
                    ...and
                    {{ manualInputValidation.invalidLines.length - 5 }} more
                  </li>
                </ul>
              </div>
            </div>
          ...
          <TabsContent value="csv" class="gap-y-3 mt-3">
            <div class="gap-y-1.5">
              <Label class="text-muted-foreground">
                {{
                  $t(
                    "campaigns.csvFormatHint",
                    'CSV must include a phone_number (or phone, mobile, number) column. Optionally include a name column. For templates with a TEXT header variable, add a "header" column. Then one column per body parameter.',
                  )
                }}
              </Label>
              ...
              >
                ...
                {{
                  isAddingRecipients
                    ...
                    : $t("campaigns.importCSV", "Import CSV")
                }}
              </Button>
            </DialogFooter>
          </TabsContent>
