  ...
  try {
    ...
    if (serverMenu?.nodes) {
      const serverConfigs = new Map(
        serverMenu.nodes.map((n: IVRNode) => [n.id, n.config]),
      );
  ...
  <div class="h-screen flex flex-col">
    ...
    >
      <Button
        ...
        size="icon"
        class="size-8"
        :aria-label="t('calling.backToFlows')"
        @click="router.push({ name: 'ivr-flows' })"
      >
