...
async function fetchLogs() {
  ...
  try {
    await store.fetchCallLogs({
      ...
      direction:
        directionFilter.value !== "all" ? directionFilter.value : undefined,
      ivr_flow_id:
        ivrFlowFilter.value !== "all" ? ivrFlowFilter.value : undefined,
...
function disconnectedByVariant(
  value: string,
    ...
    <Card v-if="!error">
      <CardContent class="pt-6">
        <div class="flex gap-4 flex-wrap items-center">
          <SearchInput
            v-model="phoneSearch"
            :placeholder="t('calling.searchByPhone')"
            class="w-48"
          />
        ...
        >
          <template #cell-caller="{ item: log }">
            <div class="cursor-pointer" @click="viewDetail(log)">
              <p class="font-medium">
                {{ log.contact?.profile_name || log.caller_phone }}
              </p>
              <p v-if="log.contact?.profile_name" class="text-muted-foreground">
                {{ log.caller_phone }}
              </p>
            </div>
          </template>
          ...
          <template #cell-duration="{ item: log }">
            <span class="inline-flex items-center gap-1.5">
              ...
              <Mic
                v-if="log.recording_s3_key"
                class="size-3.5 text-muted-foreground"
                :title="t('calling.recording')"
              />
            </span>
          </template>
          ...
          <template #cell-disconnected_by="{ item: log }">
            <Badge
              v-if="log.disconnected_by"
              :variant="disconnectedByVariant(log.disconnected_by)"
            >
              <component
                :is="disconnectedByIcon(log.disconnected_by)"
                class="size-3 mr-1"
              />
              {{
                t(
                  `calling.disconnectedBy${log.disconnected_by.charAt(0).toUpperCase() + log.disconnected_by.slice(1)}`,
                )
              }}
            </Badge>
            <span v-else class="text-muted-foreground">-</span>
      ...
      <DialogContent>
        ...
        <div v-if="selectedLog" class="gap-y-4">
          <div class="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <p class="font-medium inline-flex items-center gap-1.5">
                <PhoneIncoming
                  v-if="selectedLog.direction === 'incoming'"
                  class="size-3.5"
                />
                <PhoneOutgoing v-else class="size-3.5" />
            ...
            <div v-if="selectedLog.disconnected_by">
              <p class="text-muted-foreground">
                {{ t("calling.disconnectedBy") }}
              </p>
              <Badge
                :variant="disconnectedByVariant(selectedLog.disconnected_by)"
              >
                <component
                  :is="disconnectedByIcon(selectedLog.disconnected_by)"
                  class="size-3 mr-1"
                />
                {{
                  t(
                    `calling.disconnectedBy${selectedLog.disconnected_by.charAt(0).toUpperCase() + selectedLog.disconnected_by.slice(1)}`,
                  )
                }}
              </Badge>
            </div>
          ...
          <div v-if="selectedTransfers.length > 0" class="gap-y-2">
            ...
            <div class="gap-y-2">
              ...
              >
                ...
                <div
                  v-if="transfer.initiating_agent"
                  class="text-muted-foreground"
                >
                  {{ t("calling.transferredBy") }}:
                  <span class="text-foreground font-medium">{{
                    transfer.initiating_agent.full_name
                  }}</span>
                </div>
                <div v-if="transfer.agent" class="text-muted-foreground">
                  {{ t("calling.pickedBy") }}:
                  <span class="text-foreground font-medium">{{
                    transfer.agent.full_name
                  }}</span>
                </div>
              </div>
            </div>
          ...
          <div v-if="selectedLog.recording_s3_key" class="gap-y-2">
            ...
            />
            <p
              v-if="selectedLog.recording_duration"
              class="text-muted-foreground"
            >
