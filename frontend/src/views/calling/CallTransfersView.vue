...
import { Phone, PhoneOff, PhoneForwarded, RefreshCw, Clock } from "@lucide/vue";
...
async function fetchHistory() {
  ...
  try {
    const response = await callTransfersService.list({
      page: historyPage.value,
      limit: pageSize,
    });
    const data = response.data as any;
    historyTransfers.value = (
      data.data?.call_transfers ??
      data.call_transfers ??
      []
      ...
      <TabsList>
        <TabsTrigger value="waiting">
          ...
          <Badge
            ...
            variant="destructive"
            class="ml-2 h-5 min-w-[20px]"
          >
            {{ store.waitingTransfers.length }}
          </Badge>
        </TabsTrigger>
        <TabsTrigger value="history">{{
          t("callTransfers.history")
        }}</TabsTrigger>
      </TabsList>
          ...
          <CardContent class="pt-6">
            ...
            >
              <template #cell-caller_phone="{ item: transfer }">
                <div class="flex items-center gap-2">
                  <component
                    :is="transfer.status === 'completed' ? Phone : PhoneOff"
                    class="size-4"
                    :class="
