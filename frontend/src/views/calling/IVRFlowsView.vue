...
<template>
  <div class="p-6 gap-y-6">
      ...
      <CardContent class="pt-6">
        <Table>
          ...
          <TableBody>
            <TableRow v-for="flow in store.ivrFlows" :key="flow.id">
              <TableCell>
                <div class="cursor-pointer" @click="openEdit(flow)">
                  ...
                  <p v-if="flow.description" class="text-muted-foreground">
                    {{ flow.description }}
                  </p>
                </div>
              </TableCell>
              ...
              <TableCell>
                <div class="flex gap-1.5">
                  <Badge
                    ...
                    :aria-label="
                      flow.is_active
                        ? t('calling.toggleActiveAriaDisable', {
                            name: flow.name,
                          })
                        : t('calling.toggleActiveAriaEnable', {
                            name: flow.name,
                          })
                    "
                  ...
                  <Badge
                    ...
                    :aria-label="
                      flow.is_call_start
                        ? t('calling.toggleCallStartAriaDisable', {
                            name: flow.name,
                          })
                        : t('calling.toggleCallStartAriaEnable', {
                            name: flow.name,
                          })
                    "
              ...
              <TableCell class="text-right">
                <div class="flex justify-end gap-2">
                  ...
                  <IconButton
                    :icon="Trash2"
                    :label="
                      t('calling.deleteFlowAriaLabel', { name: flow.name })
                    "
            ...
            <TableRow
              v-if="!store.ivrFlowsLoading && store.ivrFlows.length === 0"
            >
        ...
        <div class="gap-y-4">
          ...
          <div class="gap-y-2">
            ...
            <Textarea
              ...
              :placeholder="t('calling.descriptionPlaceholder')"
              :rows="2"
            />
          </div>
        </div>
    ...
    <ConfirmDialog
      ...
      :description="
        togglingFlow?.is_active
          ? t('calling.toggleActiveConfirmDisable', {
              name: togglingFlow?.name,
            })
          : t('calling.toggleActiveConfirmEnable', { name: togglingFlow?.name })
      "
    ...
    <ConfirmDialog
      ...
      :description="
        togglingFlow?.is_call_start
          ? t('calling.toggleCallStartConfirmDisable', {
              name: togglingFlow?.name,
            })
          : t('calling.toggleCallStartConfirmEnable', {
              name: togglingFlow?.name,
            })
      "
      :confirm-label="t('common.confirm')"
