...
const needsPagination = computed(
  () => props.serverPagination && totalPages.value > 1,
);
        ...
        <TableRow v-else-if="sortedItems.length === 0">
          ...
          >
            <slot name="empty">
              <div
                v-if="emptyIcon"
                class="mb-3 mx-auto flex size-14 items-center justify-center rounded-full bg-linear-to-br from-primary/10 to-primary/5 ring-1 ring-primary/10"
              >
