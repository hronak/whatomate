    ...
    if (isOpen && props.step) {
      ...
      if (props.step.api_config?.response_mapping) {
        ...
        for (const [varName, path] of Object.entries(
          props.step.api_config.response_mapping,
        )) {
...
const responseMappingInfo = computed(() => {
  ...
  return Object.entries(props.step.api_config.response_mapping).map(
    ([varName, path]) => ({
      varName,
      path,
    }),
  ...
  <Dialog :open="open" @update:open="handleClose">
    <DialogContent class="sm:max-w-[500px]">
      <DialogHeader>
        ...
        <DialogDescription>
          Configure the mock response for this API step. The response will be
          used to simulate the API call.
        </DialogDescription>
      </DialogHeader>
