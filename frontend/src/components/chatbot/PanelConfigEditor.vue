...
function addField(
  sectionIndex: number,
  variableKey:
    string | number | bigint | Record<string, any> | null | undefined,
) {
...
<template>
  <div class="gap-y-3 min-w-0 overflow-hidden">
    ...
    <div v-else class="text-muted-foreground p-2 border rounded bg-muted/30">
      No variables captured yet. Add a Prompt node with a "Store response as"
      ...
      >
        <div class="flex items-center gap-2">
          <Input
            ...
            @update:model-value="
              (v) => setSection(sectionIdx, { label: String(v ?? '') })
            "
        ...
        <div class="flex items-center gap-3">
          <div class="flex items-center gap-1">
            ...
            <Select
              ...
              @update:model-value="
                (v) =>
                  setSection(sectionIdx, { columns: Number(v) === 2 ? 2 : 1 })
              "
            >
          ...
          <div class="flex items-center gap-1">
            <Switch
              ...
              @update:checked="
                (v) => setSection(sectionIdx, { collapsible: v })
              "
              class="scale-75"
            />
          ...
          <div v-if="section.collapsible" class="flex items-center gap-1">
            <Switch
              ...
              @update:checked="
                (v) => setSection(sectionIdx, { default_collapsed: v })
              "
              class="scale-75"
            />
          ...
          >
            <div class="flex items-center gap-1">
              ...
              <Input
                ...
                @update:model-value="
                  (v) =>
                    setField(sectionIdx, fieldIdx, { label: String(v ?? '') })
                "
              ...
              <Button
                ...
                size="icon"
                class="size-6"
                @click="removeField(sectionIdx, fieldIdx)"
              >
                <Trash2 class="size-3 text-destructive" />
              </Button>
            </div>
            <div class="flex items-center gap-2">
              <Select
                ...
                @update:model-value="
                  (v: any) =>
                    setField(sectionIdx, fieldIdx, { display_type: v })
                "
              >
              ...
              <Select
                ...
                @update:model-value="
                  (v: any) => setField(sectionIdx, fieldIdx, { color: v })
                "
              >
                <SelectTrigger class="h-6 flex-1"
                  ><SelectValue
                /></SelectTrigger>
                <SelectContent>
