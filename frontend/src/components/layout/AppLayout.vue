...
import { MessageSquare, ChevronLeft, ChevronRight, Menu, X } from "@lucide/vue";
...
<template>
  <div class="flex h-screen bg-shell">
    ...
    <!-- Mobile header -->
    <header
      class="fixed top-0 left-0 right-0 z-50 flex h-12 items-center justify-between border-b border-border bg-sidebar/95 backdrop-blur-sm px-3 md:hidden"
    >
      <RouterLink to="/" class="flex items-center gap-2">
        <div
          class="size-7 rounded-lg bg-linear-to-br from-emerald-500 to-green-600 flex items-center justify-center shadow-lg shadow-emerald-500/20"
        >
    ...
    >
      ...
      <div
        class="hidden md:flex h-12 items-center justify-between px-3 border-b border-border"
      >
        <RouterLink to="/" class="flex items-center gap-2">
          <div
            class="size-7 rounded-lg bg-linear-to-br from-emerald-500 to-green-600 flex items-center justify-center shadow-lg shadow-emerald-500/20"
          ...
          <span v-if="!isCollapsed" class="font-semibold text-foreground">
            Whatomate
          </span>
        </RouterLink>
        <Button
      ...
      <ScrollArea class="flex-1 py-2">
        <nav class="px-2" role="menubar">
          <template
            v-for="(section, sIdx) in mainSections"
            :key="section.label"
          >
            <!-- Section header -->
            <div
              ...
              :class="[
                'px-2.5 pt-4 pb-1 font-semibold uppercase tracking-wider text-foreground/45',
                sIdx === 0 && 'pt-1',
              ]"
            >
              {{ $t(section.label) }}
            ...
            <div
              v-else-if="sIdx > 0"
              :class="[
                'my-2 mx-2.5 border-t border-border',
                isCollapsed && 'mx-1',
              ]"
            />
            ...
            <div class="space-y-0.5">
              <template v-for="item in section.items" :key="item.path">
                <RouterLink
                  :to="item.path"
                  :class="[
                    'nav-active-indicator flex items-center gap-2.5 rounded-lg px-2.5 py-2 font-medium transition-colors duration-200',
                    item.active
                ...
                <template v-if="item.children && item.active && !isCollapsed">
                  <RouterLink
                    ...
                    :to="child.path"
                    :class="[
                      'flex items-center gap-2.5 rounded-lg px-2.5 py-1.5 font-medium transition-colors duration-200 ml-4',
                    ...
                    role="menuitem"
                    :aria-current="
                      route.path === child.path ? 'page' : undefined
                    "
                    @click="isMobileMenuOpen = false"
                  >
                    <component
          ...
          <template v-for="item in section.items" :key="item.path">
            ...
            <template v-if="item.children && item.active">
              <RouterLink
                ...
                :to="child.path"
                :class="[
                  'md:hidden flex items-center gap-2.5 rounded-lg px-2.5 py-1.5 font-medium transition-colors duration-200 ml-4',
                  route.path === child.path
                    ? 'bg-muted text-foreground'
                    : 'text-foreground/40 hover:text-foreground/70 hover:bg-accent',
                ]"
    ...
    >
      <div
        class="flex h-12 shrink-0 items-center gap-2 px-3 border-b border-border"
      >
        <component
          :is="activeSubNav.icon"
      ...
      <ScrollArea class="flex-1 py-2">
        <nav class="px-2" role="menu">
          <template
            v-for="(group, gIdx) in subNavGroups"
          ...
          >
            ...
            <div class="space-y-0.5">
              <RouterLink
                ...
                :to="child.path"
                :class="[
                  'nav-active-indicator flex items-center gap-2.5 rounded-lg px-2.5 py-1.5 font-medium transition-colors duration-200',
                  route.path === child.path
                    ? 'bg-muted text-foreground'
                    : 'text-foreground/50 hover:text-foreground hover:bg-accent',
                ]"
    ...
    <main
      id="main-content"
      class="flex-1 overflow-hidden pt-12 md:pt-0 bg-shell"
      role="main"
    >
      <RouterView v-slot="{ Component, route: viewRoute }">
        <component
          :is="Component"
          :key="
            viewRoute.meta.stableKey ? String(viewRoute.name) : viewRoute.path
          "
        />
      </RouterView>
      <ActiveCallPanel />
