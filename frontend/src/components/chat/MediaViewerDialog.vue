...
const currentKind = computed<Kind>(() =>
  current.value ? mediaKind(current.value) : "file",
);
const currentUrl = computed(() =>
  current.value ? mediaUrl(current.value) : "",
);
  ...
  <Dialog v-model:open="open">
    ...
    >
      ...
      <DialogDescription class="sr-only">{{
        $t("chat.mediaViewer.description")
      }}</DialogDescription>
      ...
      <div class="flex items-center gap-1 shrink-0 px-3 py-2 pr-14 text-white">
        ...
        <DialogTitle
          class="font-normal truncate max-w-[40vw]"
          :title="filename"
        >
          {{ filename || $t("chat.mediaViewer.title") }}
        </DialogTitle>
        <span v-if="total > 1" class="text-white/60 ml-1"
          >{{ index + 1 }} / {{ total }}</span
        >
      ...
      >
        ...
        <template v-else-if="current && currentKind === 'pdf'">
          ...
          <div
            v-else
            class="flex flex-col items-center gap-3 text-white/80 px-6 text-center"
          >
