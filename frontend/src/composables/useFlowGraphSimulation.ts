...
function interpolate(template: string, vars: Record<string, any>): string {
  ...
  return template.replace(TEMPLATE_RE, (_, expr: string) => {
    ...
    const value = path
      .split(".")
      .reduce<any>((acc, key) => (acc == null ? acc : acc[key]), vars);
...
export function useFlowGraphSimulation(
  ...
  async function runFrom(startId: string): Promise<void> {
    ...
    while (currentId && safety-- > 0) {
      ...
      state.currentStepIndex =
        graph.value?.nodes.findIndex((n) => n.id === currentId) ?? null;
  ...
  async function execute(node: ChatNode): Promise<string> {
    ...
    if (set && typeof set === "object") {
      for (const [k, v] of Object.entries(set)) {
        const value =
          typeof v === "string" ? interpolate(v, state.variables) : v;
        setVariable(k, value);
      }
    }
    ...
    switch (node.type) {
      ...
      case "goto_flow":
        ...
        addMessage(
          "system",
          `[goto_flow] would jump to flow ${node.config?.flow_id || "?"}`,
        );
        return "__end__";
      case "ai_response":
        addMessage(
          "system",
          "[ai_response] simulated — backend AI is not invoked in preview",
        );
        return "default";
      case "webhook":
        addMessage(
          "system",
          "[webhook] simulated — request not sent in preview",
        );
        return "default";
      default:
  ...
  function execButtons(node: ChatNode): string {
    const body = interpolate(
      stringField(node, "body", "message", "text") || node.label,
      state.variables,
    );
  ...
  function execPrompt(node: ChatNode): string {
    const body = interpolate(
      stringField(node, "body", "message", "text"),
      state.variables,
    );
  ...
  function execTiming(node: ChatNode): string {
    ...
    for (const entry of schedule) {
      ...
      const [sh, sm] = String(entry.start_time || "00:00")
        .split(":")
        .map(Number);
      const [eh, em] = String(entry.end_time || "23:59")
        .split(":")
        .map(Number);
  ...
  async function execApiCall(node: ChatNode): Promise<string> {
    ...
    addMessage(
      "system",
      `Calling API: ${node.config?.method || "GET"} ${node.config?.url || ""}`,
    );
    ...
    const fakeStep: any = {
      ...
      api_config: {
        ...
        response_mapping:
          (node.config?.response_mapping as Record<string, string>) || {},
          ...
          try {
            if (!new RegExp(regex).test(input)) {
              ...
              if (state.currentRetryCount < max) {
                addMessage(
                  "bot",
                  stringField(node, "validation_error") ||
...
function evalCondition(expression: string, vars: Record<string, any>): boolean {
  try {
    ...
    const fn = new Function(
      ...keys,
      `try { return !!(${js}); } catch (_) { return false; }`,
    );
