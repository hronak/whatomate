...
func (p *SLAProcessor) sendSLATextToCustomer(transfer models.AgentTransfer, label, message string) {
	account, err := p.app.resolveWhatsAppAccount(transfer.OrganizationID, func(u *uuid.UUID) string {
		if u == nil {
			return ""
		}
		return u.String()
...
func (p *SLAProcessor) sendChatbotReminder(contact models.Contact, settings models.ChatbotSettings) {
	...
	account, err := p.app.resolveWhatsAppAccount(contact.OrganizationID, func(u *uuid.UUID) string {
		if u == nil {
			return ""
		}
		return u.String()
...
func (p *SLAProcessor) autoCloseChatbotSession(contact models.Contact, settings models.ChatbotSettings) {
	...
	if settings.ClientInactivity.AutoCloseMessage != "" {
		if account, err := p.app.resolveWhatsAppAccount(contact.OrganizationID, func(u *uuid.UUID) string {
			if u == nil {
				return ""
			}
			return u.String()
		}(contact.WhatsAppAccountID)); err == nil {
			ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
			defer cancel()
...
func (a *App) UpdateContactChatbotMessage(contactID uuid.UUID) {
	now := time.Now()
	a.logWrite("contact chatbot timestamp", a.DB.Exec("UPDATE contacts SET chatbot_last_message_at = ?, chatbot_reminder_sent = false WHERE id = ?", now, contactID))
}
