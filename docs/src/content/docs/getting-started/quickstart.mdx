...
cp .env.example .env  # Edit to set PostgreSQL credentials and timezone
...
<Aside type="note">
  The `.env` file is required by Docker Compose. It sets PostgreSQL credentials (`POSTGRES_USER`, `POSTGRES_PASSWORD`, `POSTGRES_DB`) and the server timezone (`TZ`). Copy `.env.example` to `.env` and edit as needed.
</Aside>
