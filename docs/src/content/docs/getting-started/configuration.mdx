...
A complete commented example lives in [`config.example.toml`](https://github.com/hronak/whatomate/blob/main/config.example.toml) at the repository root — copy it to `config.toml` and edit. The reference below documents every section.
...
# google_voice_name = "en-US-Journey-F"
...
Optional, only used when an IVR step needs to synthesize speech (welcome prompt, menu read-out) and no pre-recorded audio file was uploaded.

- `provider` — one of `"openai"`, `"elevenlabs"`, `"google"`.
- `openai_key` / `openai_voice` — credentials and voice selection for OpenAI TTS API.
- `elevenlabs_key` / `elevenlabs_voice_id` — credentials and voice selection for ElevenLabs TTS API.
- `google_credentials_json` / `google_voice_name` — service account JSON string and voice selection for Google Cloud TTS.

If unset, IVR flows still work but cannot synthesize new audio prompts on the fly — only pre-recorded audio uploaded in the IVR Editor will play.
