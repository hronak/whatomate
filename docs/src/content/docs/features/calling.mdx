...
| **TTS Text** | Type text to generate audio using the configured AI provider (OpenAI, ElevenLabs, Google TTS) |
...
Whatomate supports integrating with top-tier AI voice providers (OpenAI, ElevenLabs, Google Cloud TTS) for generating highly realistic, natural-sounding IVR greetings on the fly. 

### Configure TTS

Add the corresponding provider details to your `config.toml`:

```toml
[tts]
# Choose from: "openai", "elevenlabs", "google"
provider = "openai"

# OpenAI TTS Settings
openai_key = "sk-..."
openai_voice = "alloy" # Available voices: alloy, echo, fable, onyx, nova, shimmer
...
<Aside type="tip">
  Generated audio files are cached locally in the `audio_dir` (default: `./audio`) using a SHA256 hash of the text. The same text always reuses the existing file, saving on API costs and guaranteeing instant playback for subsequent calls.
</Aside>
